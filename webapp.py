...
Обычный сайт (открывается в браузере телефона или компьютера — не обязательно
внутри Телеграма), показывает базу машин в виде таблицы, форму добавления
новой замены и персональную ссылку/QR для привязки каждого клиента к боту.
"""
...
# просто удобная подсказка. Задаётся переменной окружения PUBLIC_URL.
...
<div class="container">
  ...
  <div id="view-add" class="card">
    ...
    <div class="field">
      ...
      <div class="hint-text">Если у владельца уже есть машина в базе с таким же телефоном — новую машину привяжем к его же аккаунту.</div>
    </div>
    <div class="field">
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Посл. замена</th><th>Пробег</th><th>Масло</th><th>След. замена</th><th>Клиент</th>
          </tr>
        </thead>
...
<div class="modal-overlay" id="linkModal">
  <div class="modal">
    ...
    <button class="submit" onclick="copyLink()">Скопировать ссылку</button>
...
async function submitCar() {
  ...
  if (!payload.plate || !payload.owner_name || !payload.oil_brand || !payload.interval_months) {
    showMsg('Заполните госномер, имя, масло и интервал в месяцах.', false);
    return;
  }
  ...
  if (data.ok) {
    let extra = '';
    if (data.client_link) {
      extra = ` Персональная ссылка для клиента: ${data.client_link}`;
    }
    showMsg(`✅ Сохранено. Следующая замена ориентировочно: ${data.next_date}.${extra}`, true);
    ['plate','owner_name','owner_phone','car_model','mileage','oil_brand','notes'].forEach(id => document.getElementById(id).value = '');
  ...
  document.getElementById('table-body').innerHTML = rows.map(c => `
    <tr>
      ...
      <td>${c.telegram_id
          ...
          : `<button class="badge unlinked" onclick='openModal(${JSON.stringify(c.plate_number)}, ${JSON.stringify(c.client_link || "")})'>показать ссылку</button>`}</td>
    </tr>
  `).join('');
}

function openModal(plate, link) {
...
@app.route("/")
def index():
    return render_template_string(PAGE, brands=CAR_BRANDS)
...
def api_add():
    ...
    try:
        ...
        _, next_date = db.add_oil_change(car_id, mileage, oil_brand, interval_months, notes)
