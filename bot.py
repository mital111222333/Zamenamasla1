"""
Телеграм-бот + веб-панель системы учёта замены масла — мультитенантная версия.

Бот — общий на все точки:
1. АВТОВЛАДЕЛЕЦ (клиент) — переходит по персональной ссылке, жмёт /start —
   и сразу привязан именно к той точке, что выдала ссылку (это определяется
   по самому токену в ссылке, клиенту ничего указывать не нужно). Дальше —
   меню с историей своих машин и информацией именно о СВОЕЙ точке, плюс
   напоминания о замене масла.
2. ХОЗЯИН ТОЧКИ №1 (ADMIN_TELEGRAM_ID, из переменных окружения) — по-прежнему
   может пользоваться командами /add, /find, /broadcast прямо в Телеграме,
   как раньше. Это ЕДИНСТВЕННАЯ точка, управляемая через бота, — все
   остальные точки (добавленные через платформенную админку) работают
   только через сайт со своим логином/паролем, что и есть их основной способ
   администрирования.

Веб-панель (со входом по логину/паролю для каждой точки + отдельная
админ-панель платформы) поднимается в том же процессе (см. webapp.py).
"""

# Точка, которой управляет старый Telegram-админ (ADMIN_TELEGRAM_ID) —
# всегда id=1, потому что именно на неё указывает миграция базы данных.
...
async def shop_info_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if shop.get("lat") and shop.get("lon"):
        try:
            await update.message.reply_location(latitude=float(shop["lat"]), longitude=float(shop["lon"]))
        except ValueError:
            pass
        ...
        if ctx and ctx.get("notify_telegram_id"):
            try:
                await context.bot.send_message(
                    chat_id=int(ctx["notify_telegram_id"]),
                    text=(
                ...
                logger.error(f"Не удалось уведомить точку о брони: {e}")
...
async def add_plate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    existing = db.find_car(LEGACY_ADMIN_SHOP_ID, plate)
    if existing:
...
async def add_notes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "existing_car_id" in d:
        ...
        client = db.get_or_create_client(LEGACY_ADMIN_SHOP_ID, d["owner_name"], d.get("owner_phone"))
        car_id = db.create_or_update_car(LEGACY_ADMIN_SHOP_ID, d["plate"], client["id"], car_model=d.get("car_model"))
...
async def find_result(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    car, history = db.get_car_history(LEGACY_ADMIN_SHOP_ID, plate)
...
# ============ АДМИН: рассылка объявления/акции всем клиентам ============

async def broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        return ConversationHandler.END
    count = len(db.get_all_linked_clients(LEGACY_ADMIN_SHOP_ID))
    await update.message.reply_text(
        f"📢 Рассылка всем клиентам (сейчас привязано: {count} чел.)\n\n"
        "Напишите текст объявления/акции одним сообщением (/cancel — отменить):"
    )
...
async def broadcast_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ Отправить всем", callback_data="bc_send"),
        InlineKeyboardButton("❌ Отмена", callback_data="bc_cancel"),
    ]])
    await update.message.reply_text(
        f"Предпросмотр сообщения:\n\n{text}\n\n"
        f"Отправить {count} клиентам?",
...
async def broadcast_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await query.edit_message_text("Не нашёл текст рассылки — попробуйте /broadcast заново.")
        return
    await query.edit_message_text("⏳ Отправляю...")
    sent, failed = await send_broadcast_to_all(context, LEGACY_ADMIN_SHOP_ID, text)
    await query.edit_message_text(f"✅ Готово. Доставлено: {sent}, не удалось: {failed}.")


async def send_broadcast_to_all(context: ContextTypes.DEFAULT_TYPE, shop_id: int, text: str):
    """Шлёт текст всем привязанным клиентам ОДНОЙ конкретной точки (с небольшой
    паузой между сообщениями — лимиты Telegram)."""
    clients = db.get_all_linked_clients(shop_id)
    sent = failed = 0
    for c in clients:
        try:
            await context.bot.send_message(chat_id=c["telegram_id"], text=f"📢 {text}")
            sent += 1
        except Exception as e:
            failed += 1
            logger.warning(f"Рассылка: не удалось отправить клиенту {c['id']}: {e}")
        await asyncio.sleep(0.05)  # ~20 сообщений/сек — с запасом под лимиты Telegram
    return sent, failed


async def process_pending_broadcasts(context: ContextTypes.DEFAULT_TYPE):
    """Проверяет, не создала ли какая-то точка рассылку с сайта (кнопка
    «Отправить всем»), и если да — отправляет её СТРОГО её собственным
    клиентам, определяемым по shop_id самой рассылки."""
    pending = db.get_pending_broadcast()
    if not pending:
        return
    db.mark_broadcast_sending(pending["id"])
    sent, failed = await send_broadcast_to_all(context, pending["shop_id"], pending["message"])
    db.mark_broadcast_done(pending["id"], sent, failed)

    shop = db.get_shop(pending["shop_id"])
    notify_id = shop.get("notify_telegram_id") if shop else None
    if notify_id:
        try:
            await context.bot.send_message(
                chat_id=int(notify_id),
                text=f"📢 Рассылка с сайта завершена.\nДоставлено: {sent}, не удалось: {failed}."
            )
        except Exception:
            pass
