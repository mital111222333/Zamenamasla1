...
async def broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        return ConversationHandler.END
    count = len(db.get_all_linked_clients())
    await update.message.reply_text(
...
async def send_broadcast_to_all(context: ContextTypes.DEFAULT_TYPE, text: str):
    """Шлёт текст всем привязанным клиентам с небольшой паузой (лимиты Telegram)."""
    clients = db.get_all_linked_clients()
    sent = failed = 0
    for c in clients:
        try:
...
async def process_pending_broadcasts(context: ContextTypes.DEFAULT_TYPE):
    """Проверяет, не создал ли админ рассылку с сайта (кнопка «Отправить всем»),
    и если да — отправляет её. Бот-команда /broadcast шлёт сразу и сюда не попадает."""
    pending = db.get_pending_broadcast()
    if not pending:
        return
    db.mark_broadcast_sending(pending["id"])
    sent, failed = await send_broadcast_to_all(context, pending["message"])
